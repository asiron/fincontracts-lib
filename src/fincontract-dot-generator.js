 ...
 */
export const makeRandomIDFunc = randomFunc => () =>
  Math.floor(randomFunc() * Number.MAX_SAFE_INTEGER);
 ...
 * @extends {Visitor}
 */
export class DotGeneratorVisitor extends Visitor {
  ...
  processOneNode(node) {
    ...
    const currency = Currency.Currencies[node.currency];
...
export default class DotGenerator {
  ...
  generate(fincontract) {
    ...
    output += Object.keys(graph.labels)
      .reduce((labels, id) => `${labels}\n\t\t${id} [label="${graph.labels[id]}"];`, '');
    output += `\n\t}\n`;
    output += `}\n`;
    return output;
  }
